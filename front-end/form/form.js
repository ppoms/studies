...
function validatePasswordConfirmation(inputName, input) {
  ...
  var isValid = isEmpty(input.value) || input.value == password.value;
  return showOrHideErrorMessage(inputName, isValid, 'invalid');
}
...
function validateBirthday(inputName, input) {
  ...
  // vazio, ele deve ser considerado válido, pois a outra mensagem de erro
  // (sobre ausência/presença) já será exibida.
  var isValid = isEmpty(input.value) || /^\d{2}\/\d{2}\/\d{4}$/.test(input.value);
  return showOrHideErrorMessage(inputName, isValid, 'invalid');
}
...
function validatePhone(inputName, input) {
  ...
  var isValid = isEmpty(input.value) || /^\d{2}\d?\d{8}$/.test(input.value);
  return showOrHideErrorMessage(inputName, isValid, 'invalid');
}
...
function cleanUp(event) {
  var result = window.confirm("Tem certeza que deseja apagar o formulário?");
  if (result) {
...
/**
 * Muda fonte para itálico
 * 
 */
 
 function changeFont(event) {
  var checkbox = document.querySelector("input[type=checkbox][value=\"" + event.target.value + "\"]");
