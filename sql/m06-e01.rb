...
class MySQLExercises1
  ...
  def q_users_visits_this_year
    ...
    <<-SQL
      SELECT COUNT(name) AS num_users
      FROM users
      WHERE visited_at >= '2016-01-01'
    SQL
  end
  ...
  def q_products_count
    ...
    <<-SQL
      SELECT COUNT(id) AS num_products FROM products
    SQL
  end
  ...
  def q_products_for_sale
    ...
    <<-SQL
      SELECT COUNT(price) AS num_for_sale 
      FROM products 
  ...
  def q_products_discounts
    ...
    <<-SQL
      SELECT id, original_price, (1 - (price/original_price)) AS discount
      FROM products
    SQL
  end
  ...
  def q_sales_average_amount
    ...
    <<-SQL
      SELECT AVG(total_amount) AS average_amt
      FROM sales
    SQL
  end
  ...
  def q_sales_count_for_product
    ...
    <<-SQL
      ...
      WHERE title = 'Mochila Puma Training'
    SQL
  end
  ...
  def q_products_by_popularity
    ...
    <<-SQL
      SELECT products.id, products.title, COUNT(sales_products.id) AS num_sales
      FROM sales_products
      JOIN products ON products.id = sales_products.product_id
      GROUP BY products.id
      ORDER BY num_sales DESC, products.title ASC, products.id ASC
    SQL
  end
  ...
  def q_products_without_category
    ...
    <<-SQL
      SELECT COUNT(id) AS uncategorized
      FROM products
