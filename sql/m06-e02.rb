...
class MySQLExercises2
  ...
  def q_categories_with_parent
    ...
    <<-SQL
      SELECT p.name AS parent_name, s.name AS category_name
      FROM categories p
      JOIN categories s ON s.parent_category_id = p.id
  ...
  def q_avg_stars_per_user
    ...
    <<-SQL
      SELECT u.id, u.name, AVG(r.stars) AS avg_stars
      FROM reviews r
      JOIN users u ON u.id = r.user_id
      GROUP BY u.id
      HAVING avg_stars > 4
      ORDER BY avg_stars DESC, u.name ASC
    SQL
  end
  ...
  def q_buyers_that_are_also_sellers
    ...
    <<-SQL
      SELECT DISTINCT s.seller_id AS buyer_seller_id 
      FROM sales s
      JOIN sales b ON b.buyer_id = s.seller_id 
      ORDER BY buyer_seller_id
    SQL
  end
  ...
  def q_recently_sold_products_categories
    ...
    <<-SQL
      ...
      JOIN sales_products sp ON sp.product_id = p.id
      JOIN sales s ON sp.sale_id = s.id
      WHERE s.payment_method = 'Dinheiro'
      ORDER BY c.name
    SQL
  end
